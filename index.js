...
calcImpl = function (consumption, location, discountPrice, singleSoldPrice, source, priceType, recoveryType) {
  ...
  desList = [
    ...
    ``,
    `[具体操作]`,
    `1. 首先，我在闲鱼建一个${couponCount}券，${money+deposit}元（${money}券费+${deposit}押金），你拍下；`,
    `2. 其次，你在闲鱼建一个${returnCoupon}返券，${deposit}元，我拍下，用作押金退回；`,
    `3. 吃饭时，到九十九毡房前台去拿${couponCount}的券；`,
    `4. 消费后，把新发的券继续放到前台，双向确认交易。`,
    ``,
    `[注意事项]`,
